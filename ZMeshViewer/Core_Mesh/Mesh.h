...
namespace XMeshLib{
	...
	public:
		...
		void createMeshFromFaces(std::set<Face *> &vf, std::set<Edge *> &ve, std::vector< std::list<int> > &vb);//added

		bool isBoundary( tVertex  v ) const;
		bool isBoundary( tEdge    e ) const;
		bool isBoundary( tHalfEdge  he ) const;

		Attributes<Vertex *, Point> & Normals() { return normals;}
		...
		Point & verNormal(Vertex * v){return normals[v];}
	...
	public:
		int maxid;
		int maxfid;
		Attributes<Vertex *, Point> normals;
		Attributes<Vertex *, Point> uv;
		int sid(int vid);
		int sid2vid(int sid);
