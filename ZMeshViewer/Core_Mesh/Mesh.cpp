...
bool Mesh::write( const char outputFile[])
{
	...
	/*//write some boundaries with sharp
	std::vector< std::list<int> >::iterator vitr = v_bound.begin();
	for(;vitr!=v_bound.end();++vitr){
		std::list<int> &bound = *vitr;
		std::list<int>::iterator s_bitr = bound.begin();
		std::list<int>::iterator t_bitr = ++s_bitr;
		for(;s_bitr!=bound.end();++s_bitr){
			fprintf(fp, "%d")
			t_bitr = s_bitr;
		}
	}*/
...
//added createMeshFromFaces
void Mesh::createMeshFromFaces(std::set<Face *> &vf, std::set<Edge *> &ve, std::vector< std::list<int> > &vb){
	this->maxid = -1;
	this->maxfid = -1;
	...
	m_edges.clear();
	m_faces.clear();
	stdext::hash_map<int, int> OldInd2NewInd;
	stdext::hash_map<int, int> NewInd2OldInd;
	
	std::set<Vertex *> sv;
	std::set<Edge *> se;
	for(std::set<Face *>::iterator itr=vf.begin(); itr != vf.end(); itr++){
		HalfEdge *he = (*itr)->halfedge();
		do{
			Vertex *t = he->target();
			sv.insert(t);
			se.insert(he->edge());
			he = he->he_next();
		}while(he != (*itr)->halfedge());
	}
	//create vertices for submesh
	int vid = 1;
	for(std::set<Vertex *>::iterator itr=sv.begin(); itr != sv.end(); itr++){
...
/*	//due to uneffective createFace at this moment unable to set halfedges for each edge, I am gonna setting it myself.
	//set the boundary (vertices and edges)
	for(std::vector< std::list<int> >::iterator itr_list = vb.begin(); itr_list!=vb.end(); ++itr_list){
		std::list<int>::iterator itr = itr_list->begin();
		int old_vid0 = *itr;
		int new_vid0 = OldInd2NewInd[old_vid0];
		Vertex *v = idVertex(new_vid0);
		v->boundary() = true;
		do{
			int old_vid0 = *itr;
			int new_vid0 = OldInd2NewInd[old_vid0];
			//set boudnary vertices
			Vertex *v =  idVertex(new_vid0);
			v->boundary() = true;
			++itr;
			if(itr == itr_list->end())
			...
			//set boundary edges
			int old_vid1 = *itr;
			int new_vid1 = OldInd2NewInd[old_vid1];
			Edge *e = idEdge(new_vid0, new_vid1);
		}while(itr != itr_list->end());
	}
	// set the halfedges for each edge
	for(std::set<Edge *>::iterator itr = ve.begin(); itr!=ve.end(); ++itr){
		if(!(*itr)->boundary()){
			HalfEdge *he = (*itr)->halfedge(0);
			Vertex *t = he->source();

			HalfEdge *sym = new HalfEdge;
			t->halfedge() = sym;
			sym->vertex() = t;
			(*itr)->halfedge(1) = sym;
		}
